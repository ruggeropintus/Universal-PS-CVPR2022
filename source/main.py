...
parser.add_argument('--mode', default='TrainAndTest',
                    choices=['Train', 'Test', 'TrainAndTest'])
parser.add_argument('--agg_type', default='Transformer',
                    choices=['Transformer', 'Pooling'])
...
def main():
    ...
    trainObj, trainData, testData, logger = prepare_model_data(
        args, conf, device)
    ...
    for epoch in range(epochs):
        ...
        trainObj.run(args.mode, epoch=epoch, writer=logger, steps_per_test=200,
                     traindata=trainData, train_batch_size=args.batchsize, train_shuffle=True, train_loader_imgsize=(512, 512),
                     train_encoder_imgsize=(
                         args.encoder_imgsize, args.encoder_imgsize), train_decoder_imgsize=(512, 512),
                     testdata=testData, test_batch_size=1, test_shuffle=False, test_loader_imgsize=(512, 512),
                     test_encoder_imgsize=(256, 256), test_decoder_imgsize=(512, 512))
